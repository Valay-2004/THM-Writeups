...
We use this payload to get inof
...
==================================================
```
So now we know that there are three tables --> 1. users, 2. sqlite_sequence, 3. notes
& there are username and passwords stored so we can get them :>
...
For it we need to look at the `id` which is `1` and when we 

yeah so after using the password we got into the system as mary_ann user
```sh
